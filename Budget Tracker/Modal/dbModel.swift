...
struct SaveToDB {
    ...
    func Transactions(toDataString: String, completion: @escaping (Bool) -> ()) {
        ...
        save(dbFileURL: , toDataString: toDataString, errorLoading: "Transactions", dataType: .categories, error: { (error) in
            completion(error)
        })
    }
    
    func Categories(toDataString: String, completion: @escaping (Bool) -> ()) {
        save(dbFileURL: , toDataString: toDataString, errorLoading: "Categories", dataType: .categories, error: { (error) in
            completion(error)
        })
    }
    
    func Users(toDataString: String, completion: @escaping (Bool) -> ()) {
        save(dbFileURL: , toDataString: toDataString, errorLoading: "User Data", dataType: .non, error: { (error) in
            completion(error)
        })
    }
...
struct DeleteFromDB {
    ...
    func Transactions(toDataString: String, completion: @escaping (Bool) -> ()) {
        delete(dbFileURL: , toDataString: toDataString, error: { (error) in
               completion(error)
           })
    }
